...
public class KdfFunction {
    // We assume that only HKDF is used, because it is the only one that fits the proposed structure in RFC4210bis,
    // expecting contextInfo and allowing no salt. Thus, when it comes to customizable parameters, the only one that
    // varies is the hashing algorithm.


    // If we'll use other KDFs later, more parameters might be needed besides `digest`.
    ...
    public static KdfFunction getKdfInstance(AlgorithmIdentifier keyDerivationFunc) {
        // The source of OID definitions: https://datatracker.ietf.org/doc/html/rfc8619#section-2
        ...
        switch (keyDerivationFunc.getAlgorithm().toString()) {
            case "1.2.840.113549.1.9.16.3.28":
                return new KdfFunction(new SHA256Digest());
            case "1.2.840.113549.1.9.16.3.29":
                return new KdfFunction(new SHA384Digest());
            case "1.2.840.113549.1.9.16.3.30":
     ...
     * @param salt optional, random salt to use during key derivation; set to null if unused
     * @param context optional, raw bytes to be treated as the context of the key derivation process, set to null if unused
